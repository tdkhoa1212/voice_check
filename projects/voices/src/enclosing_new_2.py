...
class CustomFormatter(Formatter):
    """
    source: https://stackoverflow.com/questions/28868094/is-there-a-way-to-detect-zoom-in-matplotlib-plots-and-set-minor-tick-format-acco
    """
...
def straighten(filename, plot_state=True, inv_state=True):
    ...
    if plot_state:
        ...
        #------- plotting the grid frequency -----------
        axs[0].pcolormesh(t*1000, f, Sxx, shading='viridis')
        axs[0].scatter(x/per_milisec, y, c=dots, s=1, cmap='Set1')
        axs[0].grid(color='w', linestyle='--', linewidth=0.1)
        axs[0].xaxis.set_ticks(x/per_milisec, minor=True)
        axs[0].yaxis.set_ticks(y, minor=True)

        #--------set height and width--------
        axs[0].set_xlim(0, width)
        axs[0].set_ylim(0, height)

        # axs[0].set_xticks(np.linspace(0, width, num=15, dtype=np.int32))
        # axs[0].set_yticks(np.linspace(0, height, num=15, dtype=np.int32))

        axs[0].xaxis.set_major_formatter(CustomFormatter(axs[0]))
        axs[0].yaxis.set_major_formatter(CustomFormatter(axs[0]))
    ...
    clf = model()
    clf.load_weights('projects/voices/src/model.h5')
    ...
    if plot_state:
        ...
        axs[1].grid(color='w', linestyle='--', linewidth=0.1)
        axs[1].xaxis.set_ticks(x/per_milisec, minor=True)
        axs[1].yaxis.set_ticks(y, minor=True)

        #--------set height and width--------
        axs[1].set_xlim(0, width)
        axs[1].set_ylim(0, height)
        # axs[1].set_xticks(np.linspace(0, width, num=15, dtype=np.int32))
        # axs[1].set_yticks(np.linspace(0, height, num=15, dtype=np.int32))

        axs[1].xaxis.set_major_formatter(CustomFormatter(axs[1]))
        axs[1].yaxis.set_major_formatter(CustomFormatter(axs[1]))
